...
class StrReplaceEditor(BaseTool):
    ...
    parameters: dict = {
        ...
        "properties": {
            ...
            "file_text": {
                "description": "Required parameter of `create` command. The content should be provided as a base64-encoded string to ensure safe transmission. If plain text is provided, the tool will attempt to use it as-is.",
                "type": "string",
            },
            "old_str": {
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        if command == "view":
            ...
            # Try to detect base64-encoded payload and decode it. Fall back to raw text.
            import base64

            decoded_text = None
            try:
                # Remove whitespace/newlines then attempt base64 decode
                candidate = file_text.replace("\n", "").replace("\r", "")
                # Only try if length looks like base64 and characters are valid
                import re

                if len(candidate) % 4 == 0 and re.fullmatch(r"[A-Za-z0-9+/=]+", candidate):
                    decoded_bytes = base64.b64decode(candidate)
                    try:
                        decoded_text = decoded_bytes.decode("utf-8")
                    except Exception:
                        # Not utf-8 - keep original file_text
                        decoded_text = None
            except Exception:
                decoded_text = None

            final_text = decoded_text if decoded_text is not None else file_text

            await operator.write_file(path, final_text)
            self._file_history[path].append(final_text)
