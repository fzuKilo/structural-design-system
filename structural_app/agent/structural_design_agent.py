...
★★★★★ 关键规则 ★★★★★
参数模板内容【必须】写在 question 字段的文本里，【禁止】放入 context 字段。
context 字段仅用于传递警告信息、评分、图片路径等辅助信息，不用于传递参数模板。

  示例（简支梁设计缺少多个参数时）：
  ask_human(
      question="请提供简支梁的设计参数（下方数值为示例，请根据您的实际设计需求修改后提交）：\n\n跨度(m): 6\n截面宽度(m): 0.3\n截面高度(m): 0.6\n均布荷载(kN/m): 10\n材料: C30混凝土"
  )
  示例（悬臂梁设计缺少多个参数时）：
  ask_human(
      question="请提供悬臂梁的设计参数（下方数值为示例，请根据您的实际设计需求修改后提交）：\n\n悬臂长度(m): 3\n截面宽度(m): 0.3\n截面高度(m): 0.5\n均布荷载(kN/m): 10\n材料: C30混凝土"
  )
  示例（连续梁设计缺少多个参数时）：
  ask_human(
      question="请提供连续梁的设计参数（下方数值为示例，请根据您的实际设计需求修改后提交）：\n\n跨数: 2\n每跨跨度(m): 6\n截面宽度(m): 0.3\n截面高度(m): 0.5\n均布荷载(kN/m): 10\n材料: C30混凝土"
  )
  示例（框架设计缺少多个参数时）：
  ask_human(
      question="请提供框架设计参数（下方数值为示例，请根据您的实际设计需求修改后提交）：\n\n层数: 3\n跨数: 2\n各跨跨度(m): 6, 6\n各层层高(m): 4, 3.5, 3.5\n柱截面: 宽0.4 × 高0.4 m\n梁截面: 宽0.3 × 高0.6 m\n材料: C30混凝土\n楼面荷载(kN/m²): 10\n风荷载(kN/m²): 0.5（可选，不需要可删除）"
  )
  示例（桁架设计缺少多个参数时）：
  ask_human(
      question="请提供桁架设计参数（下方数值为示例，请根据您的实际设计需求修改后提交）：\n\n跨度(m): 6\n桁架高度(m): 1.2\n节间数: 5\n材料: Q235钢材\n均布荷载(kN/m): 10"
  )
- 当缺失参数 ≤ 2 个时，可逐个询问。
